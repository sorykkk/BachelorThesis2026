...
class TrainingManager:
    def __init__(
        ...
        self.scaler = GradScaler('cuda') if fp16 else None
    ...
    def one_epoch(self, training=True):
        ...
        # Inference profiling (only during validation)
        frame_times = []
        ...
        for it, batch in enumerate(loader):
            ...
            if it % print_freq == print_freq - 1 or it == len(loader) - 1:
                ...
                if self.rank == 0 or self.rank is None:
                    ...
                    if _WANDB_AVAILABLE and wandb.run is not None:
                        ...
                        wandb_log = {
                            f'{prefix}/loss': running_loss_reduced,
                            f'{prefix}/oAcc': oAcc,
                            f'{prefix}/mAcc': mAcc,
                            f'{prefix}/mIoU': mIoU,
                        ...
                        if training:
                            wandb_log['train/lr'] = self.optim.param_groups[0]["lr"]
                        else:
                            # Log per-class IoU on validation
                            if self.class_names is not None:
                                for i, s in enumerate(ious):
                                    wandb_log[f'val_iou/{self.class_names[i]}'] = 100 * s
                            wandb_log['val/best_mIoU'] = self.best_miou
                        wandb.log(wandb_log)
            ...
            if training:
                ...
                if self.fp16:
                    ...
                    loss.backward()
                if (it + 1) % self.accumulation_steps == 0 or (it + 1) == len(loader):
                    if self.fp16:
        ...
        # Return score
        if self.rank == 0 or self.rank is None:
            # Print inference profiling stats (validation only)
            if not training and len(frame_times) > 0:
                ft = np.array(frame_times)
                # Skip first batch as warmup if enough data
                ft_stats = ft[1:] if len(ft) > 10 else ft
                mean_batch_ms = float(np.mean(ft_stats)) * 1000.0
                batch_size = self.loader_val.batch_size
                mean_frame_ms = mean_batch_ms / batch_size
                ...
                print(f"    Speed (per batch): {mean_batch_ms:.2f} ms")
                print(f"    Speed (per frame): {mean_frame_ms:.2f} ms")
