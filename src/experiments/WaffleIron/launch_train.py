...
# Auto-load WANDB_API_KEY from .env.local (3 levels up from this file)
...
def distributed_training(gpu, ngpus_per_node, args, config):
    ...
    if args.gpu == 0 or args.gpu is None:
        ...
        nb_param_trainable = sum([p.numel() for p in model.parameters() if p.requires_grad]) / 1e6
        print(f"{nb_param} x 10^6 total parameters ({nb_param_trainable} x 10^6 trainable)")

        # --- Initialize wandb (rank 0 only)
        if not args.eval:
            wandb.init(
                entity='soryn-besleaga-universitatea-politehnica-timi',
                project='WaffleIron-SemanticKITTI',
                config={
                    'architecture': 'WaffleIron-48-256',
                    'dataset': args.dataset,
                    'config_file': args.config,
                    'learning_rate': config['optim']['lr'],
                    'weight_decay': config['optim']['weight_decay'],
                    'max_epochs': config['scheduler']['max_epoch'],
                    'batch_size': config['dataloader']['batch_size'],
                    'accumulation_steps': config['dataloader'].get('accumulation_steps', 1),
                    'effective_batch_size': config['dataloader']['batch_size'] * config['dataloader'].get('accumulation_steps', 1),
                    'num_workers': config['dataloader']['num_workers'],
                    'depth': config['waffleiron']['depth'],
                    'nb_channels': config['waffleiron']['nb_channels'],
                    'nb_class': config['classif']['nb_class'],
                    'num_params_M': nb_param,
                    'fp16': args.fp16,
                },
                name=f'train-{args.dataset}',
                resume='allow',
            )
    ...
    mng = TrainingManager(
        ...
        world_size=args.world_size,
        fp16=args.fp16,
        class_names=LIST_DATASETS.get(args.dataset.lower()).CLASS_NAME,
...
def get_default_parser():
    ...
    parser.add_argument(
        "--model_save_path", type=str, required=True, help="Path to save model checkpoints (ckpt_last.pth, ckpt_best.pth)"
    )
    parser.add_argument(
        "--log_path", type=str, required=True, help="Path to log folder (tensorboard, wandb, metrics)"
    )
    parser.add_argument(
