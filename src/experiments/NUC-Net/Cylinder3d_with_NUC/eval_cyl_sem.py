# -*- coding:utf-8 -*-
# Inference-time evaluation for NUC-Net (Cylinder3D with NUC + IA + NUMA) - single-scan
# Components: NUC (Non-Uniform Cylindrical partition via API),
#             IA  (Instance Augmentation - training only),
#             NUMA (Non-Uniform Multi-scale Aggregation)
# Collects: Inference Speed [ms/frame], FPS [frames/s], VRAM peak [MB],
#           mIoU, Num Params [M], FLOPS [G, param_estimate]

# USAGE:
# cd src/experiments/NUC-Net/Cylinder3d_with_NUC
# # Default (uses config's model_load_path)
# python eval_cyl_sem.py -y config/semantickitti.yaml
# # Override checkpoint path
# python eval_cyl_sem.py -y config/semantickitti.yaml -m ./model_save_dir/model.pt
# # Custom output dir
# python eval_cyl_sem.py -y config/semantickitti.yaml -o ./performance_logs
...
def main(args):
    ...
    num_scales = model_config.get('num_scales', 1)
    ...
    print(f"  Components: NUC(API) + NUMA(scales={num_scales}) + IA(OFF - eval only)")

    # ---- Build validation dataloader (single-scan, with NUC + NUMA) ----
    ...
    with torch.no_grad():
        for i_iter, (_, val_vox_label, val_grid, val_pt_labs, val_pt_fea, val_grid_ms) in enumerate(
            ...
            # --- Timed forward pass (NUC voxelization + NUMA multi-scale features) ---
    ...
    all_metrics["num_scales"] = num_scales

    # ---- Output ----
    print_backbone_metrics_table(all_metrics, dataset_name="SemanticKITTI", model_name="NUC-Net")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='NUC-Net inference evaluation with metrics profiling (single-scan)')
