...
class PanopticEval:
    ...
    # IoU thresholds for mAP computation (COCO-style: 0.50:0.05:0.95)
    MAP_THRESHOLDS = np.arange(0.50, 1.00, 0.05)
    ...
    def reset(self):
        ...
        # mAP stuff: track TP/FP/FN at multiple IoU thresholds per class
        n_thresholds = len(self.MAP_THRESHOLDS)
        self.map_tp = np.zeros((self.n_classes, n_thresholds), dtype=np.int64)
        self.map_fp = np.zeros((self.n_classes, n_thresholds), dtype=np.int64)
        self.map_fn = np.zeros((self.n_classes, n_thresholds), dtype=np.int64)
    ...
    def addBatchPanoptic(self, x_sem_row, x_inst_row, y_sem_row, y_inst_row):
        ...
        for cl in self.include:
            ...
            # --- mAP: track TP/FP/FN at multiple IoU thresholds ---
            for t_idx, threshold in enumerate(self.MAP_THRESHOLDS):
                tp_at_t = ious > threshold
                matched_gt_t = np.array([False] * unique_gt.shape[0])
                matched_pred_t = np.array([False] * unique_pred.shape[0])
                if tp_at_t.any():
                    matched_gt_t[[id2idx_gt[id] for id in gt_labels[tp_at_t]]] = True
                    matched_pred_t[[id2idx_pred[id] for id in pred_labels[tp_at_t]]] = True
                self.map_tp[cl, t_idx] += np.sum(tp_at_t)
                self.map_fn[cl, t_idx] += np.sum(np.logical_and(counts_gt >= self.min_points, matched_gt_t == False))
                self.map_fp[cl, t_idx] += np.sum(np.logical_and(counts_pred >= self.min_points, matched_pred_t == False))
    ...
    def getMAP(self, thing_classes=None):
        """Compute mean Average Precision (mAP) over thing classes.

        AP per class is computed as the mean precision across IoU thresholds
        (COCO-style AP: thresholds from 0.50 to 0.95 in steps of 0.05).

        Parameters
        ----------
        thing_classes : list of int or None
            Class indices for "thing" classes. If None, uses all included classes.

        Returns
        -------
        mAP : float
            Mean AP across thing classes.
        ap_per_class : np.ndarray of shape (n_classes,)
            AP for each class (0 for ignored/stuff classes).
        """
        if thing_classes is None:
            thing_classes = self.include

        ap_per_class = np.zeros(self.n_classes, dtype=np.double)
        for cl in thing_classes:
            # Precision at each IoU threshold: TP / (TP + FP)
            tp = self.map_tp[cl].astype(np.double)
            fp = self.map_fp[cl].astype(np.double)
            fn = self.map_fn[cl].astype(np.double)
            denom = np.maximum(tp + fp + fn, self.eps)
            # Use F1-like metric: TP / (TP + FP + FN) per threshold, then average
            precision_at_t = tp / denom
            ap_per_class[cl] = np.mean(precision_at_t)

        valid = np.array([ap_per_class[c] for c in thing_classes])
        mAP = float(np.mean(valid)) if len(valid) > 0 else 0.0
